# 部署与运维

> 将 Agent 从开发环境推向生产环境的关键实践。

---

## 为什么需要专门讨论部署

开发环境和生产环境有本质区别：
- **可靠性要求**：生产环境需要 7x24 小时稳定运行
- **性能要求**：需要处理并发请求和高负载
- **安全要求**：需要防护各种攻击和数据泄露
- **可观测性**：需要监控、日志和告警

---

## 部署架构

```
                    ┌─────────────────┐
                    │   负载均衡器     │
                    │  (Nginx/ALB)    │
                    └────────┬────────┘
                             │
         ┌───────────────────┼───────────────────┐
         │                   │                   │
         ▼                   ▼                   ▼
    ┌─────────┐        ┌─────────┐        ┌─────────┐
    │ Agent   │        │ Agent   │        │ Agent   │
    │ 实例 1  │        │ 实例 2  │        │ 实例 3  │
    └────┬────┘        └────┬────┘        └────┬────┘
         │                   │                   │
         └───────────────────┼───────────────────┘
                             │
                    ┌────────┴────────┐
                    │    共享服务      │
                    │ - Redis 缓存     │
                    │ - 向量数据库     │
                    │ - 日志服务       │
                    └─────────────────┘
```

---

## 容器化部署

### Dockerfile

下面的 Dockerfile 定义了 Agent 服务的容器镜像。关键点是 `HEALTHCHECK`——它告诉容器运行时每隔 30 秒检查一次服务是否正常，如果连续 3 次失败就重启容器。

```dockerfile
FROM python:3.11-slim

WORKDIR /app

# 安装依赖
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# 复制代码
COPY . .

# 健康检查
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD curl -f http://localhost:8000/health || exit 1

# 启动服务
CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8000"]
```

### Docker Compose

Docker Compose 用一个文件定义多个服务及其关系。下面的配置定义了三个服务：Agent API 服务、Redis 缓存和 Chroma 向量数据库。`depends_on` 指定了启动顺序，`restart: unless-stopped` 确保服务崩溃后自动重启。

```yaml
version: '3.8'

services:
  agent-api:
    build: .
    ports:
      - "8000:8000"
    environment:
      - OPENAI_API_KEY=${OPENAI_API_KEY}
      - REDIS_URL=redis://redis:6379
    depends_on:
      - redis
      - chroma
    restart: unless-stopped

  redis:
    image: redis:7-alpine
    volumes:
      - redis-data:/data

  chroma:
    image: chromadb/chroma:latest
    volumes:
      - chroma-data:/chroma/chroma

volumes:
  redis-data:
  chroma-data:
```

---

## Kubernetes 部署

### Deployment

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: agent-api
spec:
  replicas: 3
  selector:
    matchLabels:
      app: agent-api
  template:
    metadata:
      labels:
        app: agent-api
    spec:
      containers:
      - name: agent-api
        image: agent-api:latest
        ports:
        - containerPort: 8000
        resources:
          requests:
            memory: "512Mi"
            cpu: "500m"
          limits:
            memory: "1Gi"
            cpu: "1000m"
        livenessProbe:
          httpGet:
            path: /health
            port: 8000
          initialDelaySeconds: 30
          periodSeconds: 10
        readinessProbe:
          httpGet:
            path: /ready
            port: 8000
          initialDelaySeconds: 5
          periodSeconds: 5
```

### Service

```yaml
apiVersion: v1
kind: Service
metadata:
  name: agent-api
spec:
  selector:
    app: agent-api
  ports:
  - port: 80
    targetPort: 8000
  type: LoadBalancer
```

---

## 健康检查

```python
from fastapi import FastAPI
import redis
import time

app = FastAPI()

@app.get("/health")
async def health_check():
    """健康检查端点"""
    checks = {
        "status": "healthy",
        "timestamp": time.time(),
        "checks": {}
    }
    
    # 检查 Redis 连接
    try:
        r = redis.from_url(os.getenv("REDIS_URL"))
        r.ping()
        checks["checks"]["redis"] = "ok"
    except Exception as e:
        checks["checks"]["redis"] = f"error: {str(e)}"
        checks["status"] = "unhealthy"
    
    # 检查向量数据库
    try:
        # 假设使用 Chroma
        vectorstore = Chroma(persist_directory="./data")
        checks["checks"]["vectorstore"] = "ok"
    except Exception as e:
        checks["checks"]["vectorstore"] = f"error: {str(e)}"
        checks["status"] = "unhealthy"
    
    return checks

@app.get("/ready")
async def readiness_check():
    """就绪检查端点"""
    return {"status": "ready"}
```

---

## 监控与告警

### 关键指标

| 指标 | 说明 | 告警阈值 |
|------|------|---------|
| 请求延迟 | API 响应时间 | > 5 秒 |
| 错误率 | 失败请求比例 | > 5% |
| Token 使用 | LLM Token 消耗 | > 预算 80% |
| 内存使用 | 容器内存占用 | > 80% |
| CPU 使用 | 容器 CPU 占用 | > 70% |

### Prometheus 指标

```python
from prometheus_client import Counter, Histogram, start_http_server

# 定义指标
REQUEST_COUNT = Counter(
    'agent_requests_total',
    'Total requests',
    ['method', 'endpoint', 'status']
)

REQUEST_LATENCY = Histogram(
    'agent_request_duration_seconds',
    'Request latency',
    ['method', 'endpoint']
)

TOKEN_USAGE = Counter(
    'agent_token_usage_total',
    'Total token usage',
    ['model']
)

# 在请求处理中使用
@app.middleware("http")
async def metrics_middleware(request, call_next):
    start_time = time.time()
    
    response = await call_next(request)
    
    duration = time.time() - start_time
    REQUEST_COUNT.labels(
        method=request.method,
        endpoint=request.url.path,
        status=response.status_code
    ).inc()
    
    REQUEST_LATENCY.labels(
        method=request.method,
        endpoint=request.url.path
    ).observe(duration)
    
    return response
```

### 告警规则

```yaml
# Prometheus 告警规则
groups:
- name: agent_alerts
  rules:
  - alert: HighErrorRate
    expr: rate(agent_requests_total{status=~"5.."}[5m]) > 0.05
    for: 5m
    labels:
      severity: critical
    annotations:
      summary: "错误率过高"
      
  - alert: HighLatency
    expr: histogram_quantile(0.95, rate(agent_request_duration_seconds_bucket[5m])) > 5
    for: 5m
    labels:
      severity: warning
    annotations:
      summary: "请求延迟过高"
```

---

## 日志管理

### 结构化日志

```python
import structlog

logger = structlog.get_logger()

# 日志配置
structlog.configure(
    processors=[
        structlog.processors.TimeStamper(fmt="iso"),
        structlog.processors.JSONRenderer()
    ]
)

# 使用示例
logger.info(
    "agent_request",
    user_id=user_id,
    query=query,
    tokens_used=tokens,
    latency=latency
)
```

### 日志收集

```yaml
# Fluentd 配置
<source>
  @type tail
  path /var/log/agent/*.log
  pos_file /var/log/fluentd/agent.pos
  tag agent.*
  <parse>
    @type json
  </parse>
</source>

<match agent.**>
  @type elasticsearch
  host elasticsearch
  port 9200
  index_name agent
</match>
```

---

## 水平扩展

### 自动扩缩容

```yaml
# Kubernetes HPA
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: agent-api-hpa
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
    name: agent-api
  minReplicas: 2
  maxReplicas: 10
  metrics:
  - type: Resource
    resource:
      name: cpu
      target:
        type: Utilization
        averageUtilization: 70
  - type: Resource
    resource:
      name: memory
      target:
        type: Utilization
        averageUtilization: 80
```

### 会话亲和性

```nginx
# Nginx 配置
upstream agent_backend {
    ip_hash;  # 基于 IP 的会话保持
    server agent1:8000;
    server agent2:8000;
    server agent3:8000;
}
```

---

## 成本优化

### 缓存策略

```python
from functools import lru_cache
import hashlib

# 语义缓存
class SemanticCache:
    def __init__(self, threshold=0.95):
        self.cache = {}
        self.threshold = threshold
    
    def get(self, query: str):
        # 计算相似度
        for cached_query, result in self.cache.items():
            similarity = calculate_similarity(query, cached_query)
            if similarity > self.threshold:
                return result
        return None
    
    def set(self, query: str, result: str):
        self.cache[query] = result
```

### 模型选择策略

```python
def select_model(task: str, complexity: str) -> str:
    """根据任务复杂度选择模型"""
    if complexity == "simple":
        return "gpt-3.5-turbo"  # 便宜快速
    elif complexity == "medium":
        return "gpt-4-turbo"    # 平衡性价比
    else:
        return "gpt-4"          # 最高质量
```

---

## 故障恢复

### 重试机制

```python
from tenacity import retry, stop_after_attempt, wait_exponential

@retry(
    stop=stop_after_attempt(3),
    wait=wait_exponential(multiplier=1, min=1, max=10)
)
def call_llm(prompt: str) -> str:
    """带重试的 LLM 调用"""
    return llm.invoke(prompt)
```

### 降级策略

```python
def get_response(query: str) -> str:
    """带降级的响应获取"""
    try:
        # 主模型
        return primary_llm.invoke(query)
    except Exception:
        try:
            # 备用模型
            return backup_llm.invoke(query)
        except Exception:
            # 兜底响应
            return "抱歉，系统暂时无法处理您的请求"
```

---

## 检查清单

部署前检查：
- [ ] 健康检查端点正常
- [ ] 监控指标已配置
- [ ] 日志收集正常
- [ ] 告警规则已设置
- [ ] 自动扩缩容已配置
- [ ] 备份策略已实施
- [ ] 安全扫描通过

---

**上一章**: [性能优化](性能优化.md)