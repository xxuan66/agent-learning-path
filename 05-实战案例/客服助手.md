...
下面的代码实现了一个完整的客服 Agent 系统，包含四个工具：

1. `search_knowledge`：从向量数据库中搜索产品信息和常见问题
2. `check_order_status`：调用订单 API 查询物流状态
3. `process_return`：创建退换货工单
4. `escalate_to_human`：当 Agent 无法处理时，转接人工客服

系统提示词定义了 Agent 的工作流程和语气要求——这是 Agent 行为的关键控制点。
